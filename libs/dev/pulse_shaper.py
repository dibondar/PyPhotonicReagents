...
class AmplPhase2ShaperMasks_Pixelwise :
	...
	def __init__ (self, calibration_filename, pulse_shaper_resolution) :
		"""
...
class AmplPhase2ShaperMasks_SurfaceCalibration :
	...
	def __init__ (self, calibration_filename, pulse_shaper_resolution) :
		...
		#self.phase_max = 2*np.minimum(self.master_mask_offset + self.master_mask_multiplier - np.pi/2. ,
		#							self.slave_mask_offset + self.slave_mask_multiplier) 
		#self.phase_min = 2*np.maximum(self.master_mask_offset, self.slave_mask_offset + np.pi/2. )
	...
	def Initialize (self, settings) :
		...
		try : 
			...
			self.AmplPhase2ShaperMasks = AmplPhase2ShaperMasks.Initialize(  
				settings["amplphase2shapermasks"], settings["calibration_file_name"], self.npixles
			)
